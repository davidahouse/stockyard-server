 ...
 * @param {*} targets
 */
async function storeCodeCoverage(owner, repository, branch, targets) {
...
async function fetchLatestCodeCoverageSummary(owner, repository, branch) {
  ...
  if (unitTestExecution == null || unitTestExecution.rows.length == 0) {
    ...
    const targetSummary =
      "SELECT * from stockyard.codeCoverageTarget \
		WHERE code_coverage_id = $1 \
		ORDER BY target;";
    const summary = await execute("fetchCodeCoverageSummary", targetSummary, [
      coverageExecution.rows[0].code_coverage_id,
    ]);
    return {
      codeCoverageID: coverageExecution.rows[0].code_coverage_id,
