 ...
 * @param {*} defaultBranch
 */
async function storeRepository(owner, repository, defaultBranch) {
  const insert =
    "INSERT INTO stockyard.repositories (owner, repository, default_branch) \
    VALUES ($1, $2, $3) ON CONFLICT DO NOTHING;";
  return await execute("storeRepository", insert, [
    owner,
    repository,
    defaultBranch,
  ]);
...
async function fetchRepository(owner, repository) {
  const query =
    "SELECT * FROM stockyard.repositories WHERE owner = $1 \
    AND repository = $2;";
  return await execute("fetchRepositoriesWithOwner", query, [
    owner,
    repository,
  ]);
}
...
module.exports.fetchRepository = fetchRepository;
